program main
  ...
  prob_lo(:) = -0.4*(n_cell/2)
  prob_hi(:) =  0.4*(n_cell/2)
  ...
  ! build the level 1 multifab with nc component and 1 ghost cell
  call multifab_build(phi(1),la_array(1),nvar,1)
  ...
  do while ( (nl .lt. max_levs) .and. (new_grid) )
     ...
     if (new_grid) then
        ...
        call multifab_build(phi(nl+1),la_array(nl+1),nvar,1)
  ...
  do n=1,nlevs
     call multifab_build(phi(n),mla%la(n),nvar,1)
  end do
  ...
  dt =  0.0008d0! or smaller value is needed

  ! write out plotfile 0
  if (plot_int .gt. 0) then
     call write_plotfile_two(mla,phi,istep,dx,time,prob_lo,prob_hi)
  end if
